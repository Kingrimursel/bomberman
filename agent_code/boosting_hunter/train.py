...
from .callbacks import state_to_features, step_to_targets, look_for_targets, danger, influence
...
def setup_training(self):
    ...
    """
    self.strategy = NO_COIN_NO_CRATE
    ...
    self.features        = np.zeros((self.batch_size, 7), dtype=np.double)
...
def potential_coin(self, old_game_state: dict, new_game_state: dict) -> float:
    ...
    """
    if len(old_game_state['coins']) == 0 or self.mode != 'COIN':
        return .0
...
def potential_danger(self, old_game_state: dict, self_action: str, new_game_state: dict) -> float:
    ...
    old_no_danger = np.logical_not(old_danger)
    old_influence = influence(old_game_state)
    old_no_influence = np.logical_not(old_influence)
    old_position = old_game_state['self'][3]
    new_position = new_game_state['self'][3]

    # if self.mode == 'OPPONENT':
    #     step_to_escape = step_to_targets(old_arena == 0, old_position, np.argwhere(np.logical_and(old_no_danger, old_no_influence)))
    #     next_escape    = look_for_targets(old_arena == 0, old_position, np.argwhere(np.logical_and(old_no_danger, old_no_influence)))
    # else:
...
def potential_bomb(self, old_game_state: dict, self_action: str, new_game_state: dict) -> float:
    ...
    if self.mode == 'DANGER_CRATE':
        ...
        # np.logical_and(np.logical_and(old_arena == 0, old_no_danger), np.logical_not(old_opponents))
        step_to_opponent = step_to_targets(old_arena == 0, old_position, np.argwhere(old_opponents))
        next_opponent    = look_for_targets(old_arena == 0, old_position, np.argwhere(old_opponents))
        self.logger.debug(f"step_to_opponent: {step_to_opponent}, next_opponent: {next_opponent}")
        ...
        if danger_count != 0:
            ...
            if step_to_opponent == old_position:
                if self_action == 'BOMB':
                    self.logger.debug(f"Rewarded reaching tile {old_position} next to target {next_opponent} and placing a bomb.")
                    reward = SCALE_BOMB/(1-self.epsilon)
                else:
                    self.logger.debug(f"Penalized leaving tile {old_position} next to target {next_opponent} without dropping a bomb.")
                    reward = - SCALE_BOMB/(1-self.epsilon) * 10
            else:
                if self_action == 'BOMB':
                    self.logger.debug(f"Penalized for placing a bomb at {old_position} even if target {next_opponent} is not reached yet.")
                    reward = - SCALE_CRATE/(1-self.epsilon) * 4
                else:
                    if step_to_opponent == new_position:
                        self.logger.debug(f"Heading towards new crate at {next_opponent} towards {step_to_opponent}.")
                        reward = SCALE_CRATE/(1-self.epsilon)
                    else:
                        self.logger.debug(f"Penalized for not heading towards new crate at {next_opponent} towards {step_to_opponent}.")
                        reward = - SCALE_CRATE/(1-self.epsilon) * 4
...
def get_equivalent_features(feature_normalized: np.array) -> np.array:
    ...
    """
    assert feature_normalized.shape == (6, ), "Feature has not the expected size. Maybe action index was included."

    features = np.zeros((7, 6), np.double)
    ...
    f_mode        = feature[0]
    feature_tiles = feature[1:5]
    f_up_tile     = feature[1]
    f_right_tile  = feature[2]
    f_down_tile   = feature[3]
    f_left_tile   = feature[4]
    f_next_step   = feature[5]

    # Ratation invariance.
    for i in range(1, 4):
        features[i] = np.concatenate((np.array([f_mode]), np.roll(feature_tiles, i), np.array([ACTION_ROTATION[i][f_next_step]]) ))

    # Reflection invariance: Horizontal reflection.
    feature_h   = np.array([f_mode, f_down_tile, f_right_tile, f_up_tile, f_left_tile, ACTION_SYMMETRY['HORIZONTAL'][f_next_step]])
    features[4] = feature_h

    # Vertical reflection.
    feature_v   = np.array([f_mode, f_up_tile, f_left_tile, f_up_tile, f_right_tile, ACTION_SYMMETRY['VERTICAL'][f_next_step]])
    features[5] = feature_v

    # Point reflection.
    feature_p   = np.array([f_mode, f_down_tile, f_left_tile, f_up_tile, f_right_tile, ACTION_SYMMETRY['POINT'][f_next_step]])
