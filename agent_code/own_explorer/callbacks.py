...
def state_to_features(self, game_state: dict) -> np.array:
    ...
    if bomb_map[x,y]<5:#(np.count_nonzero(bomb_map < 5)>0 or np.count_nonzero(explosion_map == 1)>0):
        distance_nextsafe, closest_to_safe, safe_reachable = look_for_targets(free_space, (x, y), [(x,y) for (x,y) in np.array(np.where(bomb_map+explosion_map==5)).T], self.logger, dir=True) #distance to closest safe tile
    ...
    for num, (i,j) in enumerate([(x + h, y) for h in [-1, 1]] + [(x, y + h) for h in [-1, 1]]):
        ...
        if (arena[i,j] == 0):
            features[num]=0
        # if tile is Wall, crate, bomb, or death (bomb will explode or explosion lasts for next step)
        ...
        #Phase dependent value
                ...
                if(len(coins)>0 and features[5]==0):
                    ...
                    if((i,j)==closest_to_coin):
                        features[num]=3
                #If in danger, set way out to 3, if not in danger, set way to next crate to 3
                #TODO: implement something that gives a three to the neighbor tile with most destruction potential
                elif(features[5]==1):
                    if(bomb_map[x,y]<5 and (i,j)==closest_to_safe):
                        features[num]=3
                    elif(bomb_map[x,y]==5 and (i,j)==closest_to_crate):
                        features[num]=3
    ...
    #features[4]=1
    #wait does not lead to sure death and (can not place bomb or moving leads to sure death or bomb placement on current tile would be safe death)
    ...
    elif((features[0]!=1 or features[1]!=1 or features[2]!= 1 or features[3]!=1) and ( possible_destruction < 3)):
        features[4]=1
    elif((features[0]!=1 or features[1]!=1 or features[2]!= 1 or features[3]!=1) and (3 <= possible_destruction < 6)):
        features[4]=2
    ...
    if(bomb_map[x,y]==0 or (x,y) in bomb_xys ): #Bomb is placed on current tile, or wait is safe death
        features[4]=4
    if(features[5]==0): #If in collection mode, implicitly set so.
        features[4]=1
