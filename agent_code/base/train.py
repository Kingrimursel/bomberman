...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if(old_game_state != None and new_game_state != None):
        ...
        if len(new_coins)>0 and look_for_targets(old_free_space, (old_x, old_y), old_coins)>look_for_targets(new_free_space, (new_x, new_y), old_coins):
            events.append(APPROACH_COIN)
        if len(new_coins)>0 and look_for_targets(old_free_space, (old_x, old_y), old_coins)<=look_for_targets(new_free_space, (new_x, new_y), old_coins):
            #<= here to avoid repeated steps between two tiles with same distance to different coins
